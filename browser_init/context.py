...
from browser_init.views import BrowserError, BrowserState, TabInfo, URLNotAllowedError
...
def time_execution_sync(name: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            duration = end - start
            import logging
            logging.getLogger(__name__).debug(f'{name} took {duration:.2f}s')
            return result
        return wrapper
    return decorator
...
async def switch_to_tab(self, tab_index: int) -> None:
    ...
    if not session.context.pages:
        raise BrowserError('No pages available')
    if tab_index >= len(session.context.pages):
        raise BrowserError(f'Tab index {tab_index} out of range')
    session.current_page = session.context.pages[tab_index]
...
async def get_scroll_info(self, page: Page) -> tuple[int, int]:
    """Get scroll information about the page"""
    try:
        ...
        pixels_below = metrics['scrollHeight'] - metrics['clientHeight'] - pixels_above
        return pixels_above, pixels_below
    except Exception as e:
        logger.error(f'Failed to get scroll info: {str(e)}')
        return 0, 0
